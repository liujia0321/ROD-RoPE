# -*- coding: utf-8 -*-
"""
Run Counting-Stars acquisition/reasoning only for Mistral + Rodrope.

This is a standalone counting evaluator extracted from optimizatin_allblock_mistral.py.
...
RUN_TAG = "manual_counting_mistral"
...
def load_model_and_tokenizer(model_path: str, config: Dict[str, Any], flash_impl: str):
    ...
    if getattr(model_config, "model_type", "") == "mistral":
...
def parse_args():
    parser = argparse.ArgumentParser(description="Counting-Stars acquisition/reasoning evaluation for Mistral + Rodrope; no PPL.")
