...
def rodrope_flash_forward_two_block(
        model_self,
        query_position,
        group_size_2,
...
def rodrope_flash_forward(
    ...
    group_size_2 = int(group_size_2)
    use_far = (
        far_size is not None
        and far_query_states is not None
        and far_key_states is not None
        and int(far_size) > group_size_2
        and query_position.max() >= int(far_size)
    )
    ...
    if query_position.max() >= group_size_2:
        ...
        group_attention_mask = attention_mask[:, :group_attention_len] if not attention_mask is None else None
        group_window_size = [-1, -1]
        if use_far:
            group_window_size = [int(far_size) - group_size_2 - 1, 0]
        ...
        if use_far:
            ...
            far_attention_len = kv_seq_len - far_size
            far_attention_mask = attention_mask[:, :far_attention_len] if not attention_mask is None else None
        ...
        # normalize lse first
        neighbor_seq_length = (
            torch.full((bsz, 1), kv_seq_len, dtype=torch.long, device=query_position.device)
            if attention_mask is None
            else torch.sum(attention_mask, axis=1, keepdim=True)
        )  # [batch_size, 1]
        group_seq_length = (
            torch.full((bsz, 1), group_attention_len, dtype=torch.long, device=query_position.device)
            if attention_mask is None
            else torch.sum(attention_mask[:, :group_attention_len], axis=1, keepdim=True)
        )  # [batch_size, 1]

        # convert align left to align right and mark inactive padded rows as -inf
        ...
        neighbor_softmax_lse = _right_align_lse(neighbor_softmax_lse_right_padded, neighbor_seq_length)
        group_softmax_lse = _right_align_lse(group_softmax_lse_right_padded, group_seq_length)
        if use_far:
            far_seq_length = (
                torch.full((bsz, 1), far_attention_len, dtype=torch.long, device=query_position.device)
                if attention_mask is None
                else torch.sum(attention_mask[:, :far_attention_len], axis=1, keepdim=True)
            )
            far_softmax_lse = _right_align_lse(far_softmax_lse_right_padded, far_seq_length)
        ...
        if use_far:
            true_far_seq_max_length = far_softmax_lse.shape[-1]
            far_softmax_lse = far_softmax_lse.transpose(1, 2).unsqueeze(-1)
            mid_group_len = true_group_seq_max_length - true_far_seq_max_length

            if mid_group_len > 0:
                neighbor_mid_lse = neighbor_softmax_lse[
                    :, -true_group_seq_max_length:-true_far_seq_max_length, :, :
                ]
                group_mid_lse = group_softmax_lse[:, :mid_group_len, :, :]
                mid_denom = torch.logsumexp(
                    torch.cat([neighbor_mid_lse, group_mid_lse], dim=-1),
                    dim=-1,
                    keepdim=True,
                )
                neighbor_mid_weight = torch.exp(neighbor_mid_lse - mid_denom)
                group_mid_weight = torch.exp(group_mid_lse - mid_denom)
                attn_output[
                    :, -true_group_seq_max_length:-true_far_seq_max_length, ...
                ] = (
                    neighbor_attn_output[
                    ...
                    + group_attn_output[:, :mid_group_len, ...] * group_mid_weight
                )

            neighbor_far_lse = neighbor_softmax_lse[:, -true_far_seq_max_length:, :, :]
            group_far_lse = group_softmax_lse[:, -true_far_seq_max_length:, :, :]
            far_denom = torch.logsumexp(
                torch.cat([neighbor_far_lse, group_far_lse, far_softmax_lse], dim=-1),
                dim=-1,
                keepdim=True,
            )
            neighbor_far_weight = torch.exp(neighbor_far_lse - far_denom)
            group_far_weight = torch.exp(group_far_lse - far_denom)
            far_weight = torch.exp(far_softmax_lse - far_denom)
            attn_output[:, -true_far_seq_max_length:, ...] = (
                neighbor_attn_output[:, -true_far_seq_max_length:, ...] * neighbor_far_weight
                + group_attn_output[:, -true_far_seq_max_length:, ...] * group_far_weight
                + far_attn_output[:, -true_far_seq_max_length:, ...] * far_weight
            )
        else:
            lse_gap = group_softmax_lse - neighbor_softmax_lse[:, -true_group_seq_max_length:, :, :]
            neighbor_group_weight = 1 / (1 + torch.exp(lse_gap))
            group_weight = 1 / (1 + torch.exp(-lse_gap))

            attn_output[:, -true_group_seq_max_length:, ...] = (
                neighbor_attn_output[:, -true_group_seq_max_length:, ...] * neighbor_group_weight
                + group_attn_output[:, -true_group_seq_max_length:, ...] * group_weight
            )
